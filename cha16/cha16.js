...
function overlap(actor1, actor2) {
    return (actor1.pos.x + actor1.size.x) > actor2.pos.x &&
           ...
           (actor1.pos.y + actor2.size.y) > actor2.pos.y &&
            actor1.pos.y < actor2.size.y + actor2.size.y;
}
...
Coin.prototype.update = function(time){
    // wobble property is incremented to track time
    let wobble = this.wobble + time *wobbleSpeed;
    // wobble then used as an argument to Math.sin to 
    // find the new position on the wave.
...
Player.prototype.update = function(time, state, keys){
    let xSpeed = 0;
    if (keys.ArrorLeft) xSpeed -= playerXSpeed;
    if (keys.ArrorRight) xSpeed += playerXSpeed;
    let pos = this.pos;
...
function runAnimation(framFunc) {
    let lastTime = null;
    function frame(time) {
        if (lastTime != null){
            let timeStep = Math.min(time - lastTime, 100) / 1000;
            if (framFunc(timeStep) === false) return;
        }
        lastTime = time;
